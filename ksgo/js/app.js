(() => {
    ...
    const menuIconLabel = document.querySelector(".menu__label");
    let isMoved = false;
    menuIcon.addEventListener("click", (() => {
        if (window.innerWidth > 450.98) if (!isMoved) {
            header.appendChild(menuIcon);
            menuIconLabel.remove();
            isMoved = true;
        } else {
            menu.insertBefore(menuIcon, menu.firstElementChild);
            menu.appendChild(menuIconLabel);
            isMoved = false;
        }
    }));
    window["FLS"] = true;
